...
func openGZReader() (gzReader, error) {
	// Get the directory that contains the access.log and all of the compressed
	// historical logs.
	files, err := os.ReadDir("/home/user/skynet-webportal/docker/data/nginx/logs/")
	if err != nil {
	...
	gzReader.archiveLengths, err := os.Open("/home/user/metrics/nginxAccesLogArchiveOffsets.dat")
	if err != nil {
		return errros.AddContext(err, "unable to open archive offsets file")
	}
	offsetsData, err := ioutil.ReadAll(gzReader.archiveLengths)
	if err != nil {
		return errors.AddContext(err, "unable to read archive offsets file")
	}
...
func (gzr *gzReader) Seek(offset int64, whence int) (int64, error) {
	...
	for {
		...
		if gzr.fileIndex >= len(gzr.archiveList) {
			liveFile, err := os.Open("/home/user/skynet-webportal/docker/data/nginx/logs/access.log")
			if err != nil {
		...
		if len(gzr.archiveLengths) > gzr.fileIndex && gzr.logicalOffset+archiveLengths[gzr.fileIndex] < offset {
			gzr.logicalOffset += archiveLengths[gzr.fileIndex]
			gzr.fileIndex++
			continue
		}
		...
		archivePath := filepath.Join("/home/user/skynet-webportal/docker/data/nginx/logs", gzr.archiveList[gzr.fileIndex])
		...
		for {
			...
			if err == io.EOF {
				...
				// archiveLengths file.
				gzr.logicalOffset += n
				archiveLengths = append(archiveLengths, gzr.progressThroughCurrentArchive)
				byteLength := make([]byte, 8)
				binary.LittleEndian.PutUint64(byteLength, gzr.progressThroughCurrentArchive)
				_, err := gzr.archiveLengths.Write(byteLength)
				if err != nil {
...
func (gzr *gzReader) Read(p []byte) (int, error) {
	...
	for {
		...
		for {
			...
			gzr.progressThroughCurrentArchive += n
			p = p[n:]
			if err == io.EOF && gzr.fileIndez == len(gzr.archiveList) {
